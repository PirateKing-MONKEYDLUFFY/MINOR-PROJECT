...
export default function Medicines() {
  ...
  const [managedElders, setManagedElders] = useState<{ id: string, full_name: string }[]>([]);
  ...
  }, [profile, authLoading, selectedElderId]);
  ...
  const handleAddMedicine = async (e: React.FormEvent) => {
    ...
    try {
      const targetElderId = profile.role === "caregiver" ? selectedElderId : profile.id;
      
      if (!targetElderId) {
        toast({ title: "Error", description: "No elder selected", variant: "destructive" });
        return;
      }
  ...
  if (!profile || (profile.role === "caregiver" && managedElders.length === 0)) {
    return (
      <div className="min-h-screen bg-background">
        <Header onMenuToggle={() => setShowMenu(!showMenu)} showMenu={showMenu} />
        <main className="container py-12 text-center">
          ...
          <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <p className="text-sm text-muted-foreground">
              {profile.role === "caregiver" 
                ? `Managing for ${managedElders.find(e => e.id === selectedElderId)?.full_name || "Elder"}`
                : `Managing for ${profile.full_name}`}
            </p>
          </motion.div>

          <div className="flex gap-2">
            {profile.role === "caregiver" && managedElders.length > 1 && (
              <select 
